...
if __name__ == '__main__':
    ...
    if choice == "Prediction":
        st.header("Know the prediction of Death Rate")
        ...
        # Remove hyphens and convert to float
        date_no_hyphen = formatted_date.replace('-', '')
        date_float = float(date_no_hyphen)
        # Prepare the input array for prediction
        x = np.array([date_float]).reshape(-1, 1)

        # Load your training data and labels
        x_train = np.array([[1], [2], [3], [4], [5]])  # Example input training data
        y_train = np.array([2, 4, 6, 8, 10])  # Example target training data

        # Create polynomial features
        poly_features = PolynomialFeatures(degree=3)
        x_poly_train = poly_features.fit_transform(x_train)
        x_poly_pred = poly_features.transform(x)
        # Fit the polynomial regression model
        poly_regression = LinearRegression()
        poly_regression.fit(x_poly_train, y_train)

        # Make a prediction
        pred = poly_regression.predict(x_poly_pred)[0]
